...
class AqtConvBlock(nn.Module):
    ...
    is_last: bool = False
    lhs_bits: Optional[int] = 4
    rhs_bits: Optional[int] = 4
...
def AqtResNet18(n_classes: int, rhs_bits=4, lhs_bits=4):
    return AqtResNet(
        STAGE_SIZES[18], 
        AqtResNetBlock, 
        n_classes, 
...
def save_model_params(state, filename):
    """모델 파라미터를 지정된 폴더에 파일로 저장합니다."""
    # 현재 실행 중인 스크립트의 디렉토리 경로를 가져옵니다
...
def train_model(num_epochs, batch_size, learning_rate, file_prefix, lhs_bits=16, rhs_bits=16):
    """
    train_model(num_epochs, batch_size, learning_rate, lhs_bits=4, rhs_bits=4):
    ...
    for epoch in range(num_epochs):
        ...
        # 10 에폭마다 모델 저장 (또는 원하는 주기로 설정)
        if (epoch + 1) % 10 == 0:
            save_model_params(state, f"{file_prefix}_epoch_{epoch+1}.pkl")
        
    save_model_params(state, f"{file_prefix}_final.pkl")

    return state

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train AqtResNet18 model on CIFAR-10")
    parser.add_argument("--epochs", type=int, default=100, help="Number of epochs to train")
    parser.add_argument("--batch_size", type=int, default=64, help="Batch size for training")
    parser.add_argument("--learning_rate", type=float, default=1e-3, help="Learning rate for training")
    parser.add_argument("--file_prefix", type=str, default="aqt_resnet18", help="Prefix for saved model files")
